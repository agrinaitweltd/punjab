...
assert.doesNotMatch(customersPage, /Both Invoice & Credit Note/)
...
assert.doesNotMatch(parserSource, /Please check the credit note number/)
const adminPortal = fs.readFileSync(new URL('../src/pages/admin/AdminPortal.tsx', import.meta.url), 'utf8')
assert.match(adminPortal, /creditNumber:\s*sourceCreditNumber\s*\|\|\s*undefined/)
const creditNoteService = fs.readFileSync(new URL('../src/services/creditNoteService.ts', import.meta.url), 'utf8')
assert.match(creditNoteService, /input\.creditNumber\?\.trim\(\)\s*\|\|\s*nextCreditNumber\(existing\)/)
