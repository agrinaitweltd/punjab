...
import { verifySalesLogin } from "../../lib/salesmen"
...
export function SalesLoginPage({ onLogin }: { onLogin: (salesman: Salesman) => void }) {
  ...
  const submit = (e: FormEvent) => {
    e.preventDefault()
    const salesman = verifySalesLogin(number, code)
    if (!salesman) { setError("Salesman number or code is incorrect."); return }
    setError("")
    onLogin(salesman)
  }
    ...
    <div className="stack" style={{ maxWidth: 380, margin: "60px auto" }}>
      <div style={{ textAlign: "center" }}>
        ...
        <p style={{ fontSize: 13.5, color: "#6b7a70", marginTop: 3 }}>
          Enter your salesman number and code to access the Sales module.
        </p>
      </div>
      <form onSubmit={submit} className="ps-table-card" style={{ padding: 24 }}>
        <label className="form-control">
          <span>Salesman Number</span>
          <input type="text" inputMode="numeric" placeholder="e.g. 16" value={number} onChange={e => setNumber(e.target.value)} autoFocus required />
        </label>
        <label className="form-control">
          <span>Code</span>
