...
export type CommunicationDeliveryLog = {
  ...
  html?: string; providerMessageId?: string; replyTo?: string; createdBy?: string; idempotencyKey?: string
...
export async function getCommunicationDeliveryLogs(): Promise<CommunicationDeliveryLog[]> {
  ...
  return (data || []).map(row => ({
    ...
    html: row.html || undefined, providerMessageId: row.provider_message_id || undefined, replyTo: row.reply_to || undefined, createdBy: row.created_by || undefined, idempotencyKey: row.idempotency_key || undefined,
    attachmentNames: Array.isArray(row.payload?.attachmentNames) ? row.payload.attachmentNames : [],
  }))
}
